...
| 构建 | Go 1.26+ + vendor + rpmbuild（openEuler 交付） | goamd64=v1, CGO_ENABLED=0 |
...
- 生成 vendor tarball: `bash packaging/scripts/prepare-vendor.sh <version>`
- 生成完整发布产物: `bash packaging/scripts/prepare-release.sh <version>`
- RPM 本地构建验证: `rpmbuild -ba packaging/euler-copilot-shell.spec`（需在 openEuler 环境）
- 格式化: `go fmt ./...`
...
- `internal/server/` — opencode serve 生命周期管理
...
- `packaging/` — RPM 打包相关文件
  - `packaging/euler-copilot-shell.spec` — RPM spec 文件（Vendor + Go 工具链捆绑方案）
...
- 运行 `go build`, `go test`, `golangci-lint`, `shellcheck`, `go fmt`
  （⚠️ 构建产物必须通过 `scripts/build.sh` 输出到 `build/` 目录，详见 `witty-build` skill）
- 更新 `*_test.go` 文件
...
- 在 Bash 模板中使用 `{{ }}` 分隔符
- 引用不存在的依赖或虚构的 API
- 将构建产物写到 `build/` 以外（`/tmp/`、项目根目录、随机文件名）；所有构建必须通过 `scripts/build.sh`
...
以下 skill **不是懒加载**——只要任务涉及对应操作，Agent **必须**在执行该操作之前**完整加载** SKILL.md body 并严格遵循其指令：

- **`witty-build`**：任何涉及 `go build`、`go test`、`go vet`、`golangci-lint`、`bash scripts/build.sh`、
  openEuler 远程编译/测试、试运行二进制的操作，**必须先完整读取** `.agents/skills/witty-build/SKILL.md`。
  该 skill 定义了构建铁律（两步编译、产物只能写 `build/`、禁止写 `/tmp/`）、试运行要求等。
  **违反此规则（如将产物写到 `/tmp/` 或跳过 `scripts/build.sh`）视为严重错误。**

  判断标准：如果你即将在终端中执行任何 `go build`、`go test`、`bash scripts/build.sh` 命令（含远程执行），
  先停下来加载 `witty-build` skill。
