...
interact_question_save_python_code = _("是否保存 Python 代码？")
interact_question_input_file_name = _("请输入文件名：")
...
backend_framework_auth_invalid_api_key = _(
    "{brand_name} 智能体 API 密钥无效，请检查配置文件\n\n" + backend_check_config_msg + "\n"
)
...
backend_framework_request_exceptions = _(
    "{brand_name} 智能体请求异常，请检查网络连接\n\n" + backend_check_config_msg + "\n"
)
...
config_md_title = _("当前配置")
config_md_header_key = _("设置项")
config_md_header_value = _("值")
config_entry_backend = _("大模型后端")
config_entry_query_mode = _("问答模式")
config_entry_advanced_mode = _("启用高级模式")
config_entry_debug_mode = _("启用调试模式")
config_entry_spark_app_id = _("星火大模型 App ID")
config_entry_spark_api_key = _("星火大模型 API Key")
config_entry_spark_api_secret = _("星火大模型 API Secret")
config_entry_spark_url = _("星火大模型 URL")
config_entry_spark_domain = _("星火大模型领域")
config_entry_framework_url = _("{brand_name} 智能体 URL")
config_entry_framework_api_key = _("{brand_name} 智能体 API Key")
config_entry_model_url = _("OpenAI 模型 URL")
config_entry_model_api_key = _("OpenAI 模型 API Key")
config_entry_model_name = _("OpenAI 模型名称")
config_interact_query_mode_forbidden = _("当前后端无法使用{mode}模式")

init_welcome_msg = _("欢迎使用 {brand_name} 智能体")
init_welcome_usage_guide = _("使用方法：输入问题，按下 Ctrl+O (字母 O) 提问")
init_welcome_help_hint = _('更多用法详见命令行帮助："copilot --help"')
init_welcome_docs_link = _("使用指南：{url}")
init_welcome_alpha_warning = _(
    "{brand_name}（内测版）旨在让内测用户提前体验 \
...
init_framework_api_key_notice_title = _("获取 {brand_name} 智能体 API Key")
init_framework_api_key_notice_content = _("请前往 {url}，点击右上角头像图标获取 API Key")

query_mode_chat = _("智能问答")
query_mode_shell = _("智能 Shell")
query_mode_plugin = _("智能插件")
