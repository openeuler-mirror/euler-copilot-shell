...
如果这次集成测试需要在 openEuler 远程环境中执行，先直接读取 `.agents/config.yaml` 获取连接方式；不要先依赖 `find_path` 或目录扫描判断该文件是否存在。该文件可能被 `.gitignore` 隐藏，但仍然真实存在。若直接读取失败，再回退参考 `.agents/config.template.yaml`。
