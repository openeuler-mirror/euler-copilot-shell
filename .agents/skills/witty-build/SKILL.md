...
2. 直接尝试读取已知路径 `.agents/config.yaml`。
3. 如果直接读取失败，再回退读取 `.agents/config.template.yaml`，并明确提示开发者补齐本地配置。
...
Agent 根据 `.agents/config.yaml` 自动连接远程 openEuler 环境并执行最终验证。开始前先**直接读取** `.agents/config.yaml`。
