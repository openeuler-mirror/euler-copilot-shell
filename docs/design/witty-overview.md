...
Shell 集成通过 RPM 安装 `/etc/profile.d/witty.sh` 默认对所有用户生效，无需用户手动配置。用户若不想要，在 `~/.bashrc` 中加一行即可关闭：

```bash
export WITTY_SHELL_ENABLE=0
```

RPM 安装/卸载不修改系统 bashrc 或用户 rc 文件，卸载即移除集成，零残留。详见 [shell-adapter.md §8.1](shell-adapter.md#81-集成入口)。
...
## 7. OpenCode Server 生命周期管理

witty 通过 `internal/server` 模块自动管理 `opencode serve` 进程的完整生命周期，包括启动、检测、复用和跨会话持久化。

详细设计见 [server-lifecycle.md](server-lifecycle.md)，核心要点：

- **自动启动**：witty 首次使用时自动 spawn `opencode serve` 子进程，无需用户手动操作。
- **自动检测**：通过 TCP 端口探测 + `/global/health` 端点验证，自动判断是否已有可用 server。
- **跨会话复用**：witty 退出后 server 守护运行，状态持久化到 `~/.config/witty/server-state.json`，下次启动时无缝复用。
- **HTTP Basic Auth**：通过随机生成的 `OPENCODE_SERVER_PASSWORD` 实现多用户隔离和身份识别。
- **非固定端口**：默认端口被占用时自动选择下一个可用端口。

用户可通过 `WITTY_SERVER_AUTO_START=false` 环境变量或配置项 `server.auto_start = false` 关闭自动启动，回退到当前手动模式。

---

## 8. OpenCode Server API 约定
...
### 9.2 策略：Phase 1 块级缓冲 + 边界渲染，Phase 2 即时回显增强
...
- `witty init bash`
- Shell Adapter（DEBUG trap + extdebug）
- 全局集成（`/etc/profile.d/witty.sh`，RPM 安装/卸载）
...
- RPM 安装后所有用户默认生效；`WITTY_SHELL_ENABLE=0` 可关闭；卸载零残留
...
- Server 自动启动与生命周期管理（`internal/server`）
- 配置文件与日志
- 主题与配色
- 诊断命令 `witty doctor`
- 包管理 / 发布流程
