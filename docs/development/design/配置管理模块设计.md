...
graph TB
    ...
    subgraph "持久化存储"
        J[JSON序列化<br/>to_dict/from_dict] --> K[配置文件<br/>witty-assistant.json]
        K --> L[用户目录<br/>~/.config/witty/]
    end
...
classDiagram
    class ConfigManager {
        ...
        +get_witty_url() str
        +set_witty_url(url: str) None
        +get_witty_key() str
        +set_witty_key(key: str) None
        +get_default_app() str
...
配置文件位置: `~/.config/witty/witty-assistant.json`

```mermaid
graph LR
    subgraph "配置文件结构"
        A[witty-assistant.json] --> B[backend: 后端选择]
        A --> C[openai: OpenAI配置]
        A --> D[witty: Hermes配置]
        A --> E[log_level: 日志级别]
...
classDiagram
    class DefaultValues {
        ...
        +HERMES_API_KEY: ""
        +HERMES_DEFAULT_APP: ""
        +DEFAULT_BACKEND: Backend.SYSAGENT
